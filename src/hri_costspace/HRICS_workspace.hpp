...
class Workspace : public HumanAwareMotionPlanner
{
 ...
 public:
   ...
   */
  double* testTransferPointToTrajectory(const Eigen::Vector3d& WSPoint,
  ...
  /**
   * Compute a transfert point from a loaded grid, without taking into acount a
   * possible colision.
   * If a trandfert point is found, return true and put the vector into
   * transfPoint else return false
   */
  bool computeBestTransferPoint(Eigen::Vector3d& transfPoint);

  /**
   * Compute a transfert point from a loaded grid, taking into acount a possible
   * colision.
   * If a trandfert point is found, return true and put the vector into
   * transfPoint else return false
   */
   ...
   * the second one allow to choose different computing fonctions taking into
   * account or not the environment
   */
  bool ComputeTheObjectTransfertPoint(bool Move,
                                      int type,
