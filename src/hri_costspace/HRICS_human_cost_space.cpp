...
//! @param The cell size, each human will be associated a grid (the cell size is
...
//! Each elemetary costspace holds the method used to compute the cost
//associated
...
void HumanCostSpace::deleteElementarySpaces() {
  if (m_DistanceSpace) delete m_DistanceSpace;

  if (m_VisibilitySpace) delete m_VisibilitySpace;
...
//! @brief Initialize the Human Grids composed of the different elementary
//costspace
...
bool HumanCostSpace::initHumanGrids(double cellsize) {
  ...
  for (int i = 0; i < int(m_Humans.size()); i++) {
    m_Grids.push_back(new AgentGrid(cellsize,
                                    envsize,
                                    m_Humans[i],
                                    m_DistanceSpace,
                                    m_VisibilitySpace,
...
bool HumanCostSpace::initPr2() {
  ...
  if (m_planning_type == NAVIGATION) {
    for (int i = 0; i < int(m_Robot->getNumberOfJoints()); i++) {
      ...
      if (name == "platformJoint" || name == "Torso") {
        m_CostJoints.push_back(m_Robot->getJoint(i));
        cout << "Add joint : " << m_CostJoints.back()->getName() << endl;
      }
    }
  ...
  if (m_planning_type == MANIPULATION) {
    for (int i = 0; i < int(m_Robot->getNumberOfJoints()); i++) {
      ...
      if (name == "right-Arm4" || name == "right-Arm5" ||
          name == "right-Arm6" || name == "right-Arm7" ||
  ...
  if (m_planning_type == MOBILE_MANIP) {
    for (int i = 0; i < int(m_Robot->getNumberOfJoints()); i++) {
      ...
      if (name == "platformJoint" || name == "Torso" ||
          ...
          // name == "right-Arm3" ||
          name == "right-Arm4" ||
          name == "right-Arm5" || name == "right-Arm6" ||
          name == "right-Arm7" || name == "fingerJointGripper_0" ||
          name == "left-Arm6" || name == "left-Arm7") {
        m_CostJoints.push_back(m_Robot->getJoint(i));
        cout << "Add joint : " << m_CostJoints.back()->getName() << endl;
      }
    }
...
bool HumanCostSpace::initJustin() {
  ...
  if (m_planning_type == MANIPULATION) {
    for (int i = 0; i < int(m_Robot->getNumberOfJoints()); i++) {
      ...
      if (/*name == "RightArm1" ||
          ...
          //         name == "LeftArm2" ||
          //         name == "LeftArm3" ||
          name == "LeftArm4" || name == "LeftArm5" || name == "LeftArm6" ||
          //         name == "LeftArm7" ||
          name == "LeftWrist") {
        m_CostJoints.push_back(m_Robot->getJoint(i));
        cout << "Add joint : " << m_CostJoints.back()->getName() << endl;
  ...
  if (m_planning_type == MOBILE_MANIP) {
    for (int i = 0; i < int(m_Robot->getNumberOfJoints()); i++) {
      ...
      if (name == "Platform" || name == "base2" || name == "LeftArm3" ||
          name == "LeftArm4" || name == "LeftArm5" || name == "LeftArm6" ||
          name == "LeftArm7" || name == "LeftWrist") {
        m_CostJoints.push_back(m_Robot->getJoint(i));
        cout << "Add joint : " << m_CostJoints.back()->getName() << endl;
      }
    }
...
void HumanCostSpace::saveAgentGrids() {
  for (unsigned int i = 0; i < m_Grids.size(); i++) {
    ...
    std::string filename =
        "../assets/Cost3DGrids/human_grids_" + oss.str() + ".grid";
...
void HumanCostSpace::loadAgentGrids(const std::string& filename) {
  ...
  AgentGrid* agentGrid = new AgentGrid(
      m_Humans[0], m_DistanceSpace, m_VisibilitySpace, m_ReachableSpace);
...
void HumanCostSpace::drawReachableGrid() {
  if (m_ReachableCells.empty()) return;
...
bool HumanCostSpace::getHandoverPointList(std::vector<Eigen::Vector3d>& points,
  ...
  if (recompute_cells) {
    ...
    if (arm_type == 1) {
      reachable_cells =
          m_ReachableSpace->getGrid()->getAllReachableCellsOneArm(true);
    }
    if (arm_type == 2) {
      reachable_cells =
          m_ReachableSpace->getGrid()->getAllReachableCellsOneArm(false);
    }

    double max = ENV.getDouble(Env::Kdistance) +
    ...
    for (int i = 0; i < int(reachable_cells.size()); i++) {
      ...
      double d_cost = ENV.getDouble(Env::Kdistance) *
                      m_DistanceSpace->getWorkspaceCost(point);
      double v_cost = ENV.getDouble(Env::Kvisibility) *
                      m_VisibilitySpace->getWorkspaceCost(point);
      double r_cost = ENV.getDouble(Env::Kreachable) *
                      m_ReachableSpace->getWorkspaceCost(point);

      m_ReachableCells.push_back(
          make_pair((d_cost + v_cost + r_cost) / max, reachable_cells[i]));
    }
