...
bool HRICS_init_human_trajectory_cost()
{
    ...
    if( global_human_traj_features == NULL )
    {
        ...
        {
            global_motionRecorders.push_back(
                        new HRICS::RecordMotion( passive_agent ) );
            global_motionRecorders.push_back(
                        new HRICS::RecordMotion( active_agent ) );
            ...
            if( passive_agent->getJoint( "rShoulderZ" ) != NULL )
            {
                ...
                std::string foldername = matlabfolder +
                        "kinect_good_motions/human_one_good/";
            ...
            else if ( load_a_term )
            {
                     ...
                     global_motionRecorders[0]->getStoredMotions().size(); i++)
                {
                    cout << global_motionRecorders[0]->getStoredMotionName(i)
                            << endl;
                    cout << global_motionRecorders[1]->getStoredMotionName(i)
                            << endl;
                }
            }
            else
            {
                ...
                if( dataset == user_study )
                {
                    ...
                    global_motionRecorders[0]->loadCSVFolder(
                                foldername + "human_two/", quiet );
                    global_motionRecorders[1]->loadCSVFolder(
                                foldername + "human_one/", quiet );
                }
                ...
                else if( dataset == human_robot_experiment )
                {
                    ...
                    global_motionRecorders[0]->loadTrajectories(
                                foldername, false );
                    global_motionRecorders[1]->loadCSVFolder(
                                foldername, false );
                }
        ...
        global_humanPredictionCostSpace =
                new HRICS::HumanPredictionCostSpace( active_agent,
                                                     occupancyGrid );
        // Human trajectory costspace
        global_human_traj_features = new HRICS::HumanTrajFeatures(
                    active_agent, passive_agent );

        // Set active joints and joint bounds
        global_human_traj_simulator = new HRICS::HumanTrajSimulator(
        ...
        Move3D::global_costSpace->addCost(
                    "costHumanWorkspaceOccupancy", boost::bind(
                        &HRICS::HumanPredictionCostSpace::getCurrentOccupationCost,
                        global_humanPredictionCostSpace, _1) );

        Move3D::global_costSpace->addCost(
                    "costHumanTrajectoryCost", boost::bind(
                        &HumanTrajFeatures::cost,
                        global_human_traj_features, _1) );
    }
    ...
    cout << " global_human_traj_features : "
         << global_human_traj_features
         << endl;
...
HumanTrajFeatures::HumanTrajFeatures( Move3D::Robot* active,
    ...
    smoothness_feat_( active,
                      active->getName() ==
                      "HERAKLES_HUMAN2" ? active->getJoint("rWristX") :
    ...
    collision_feat_( active )
{
    ...
    visi_feat_.setWeights( Move3D::WeightVect::Ones(
                               visi_feat_.getNumberOfFeatures() ) );

    musc_feat_.setActiveDoFs( active_dofs_ );
    musc_feat_.setWeights( Move3D::WeightVect::Ones(
                               musc_feat_.getNumberOfFeatures() ) );

    smoothness_feat_.setActiveDoFs( active_dofs_ );
    smoothness_feat_.setWeights( Move3D::WeightVect::Ones(
                                     smoothness_feat_.getNumberOfFeatures() ) );
...
bool HumanTrajSimulator::init()
{
    ...
    // set is human
    is_active_agent_human_ = human_active_->getName().find("HUMAN")
            != std::string::npos;
    is_passive_agent_human_ = human_passive_->getName().find("HUMAN")
            != std::string::npos;
    ...
    {
        global_DrawModule->addDrawFunction( "HumanSimulator",
                                            boost::bind(
                                                &HumanTrajSimulator::draw,
                                                this) );
...
void HumanTrajSimulator::draw()
{
    ...
    {
        int nb_frames = 15;
        int id_of_demo_to_draw =
                HriEnv->getInt(HricsParam::ioc_spheres_to_draw);

        cout << "id_of_demonstration_ : " << id_of_demo_to_draw << endl;

        motion_recorders_[0]->drawMotion(
                    human_1_motions_[id_of_demo_to_draw], nb_frames );
        motion_recorders_[1]->drawMotion(
                    human_2_motions_[id_of_demo_to_draw], nb_frames );
    }
}
                                          ...
                                          Move3D::confPtr_t q_tmp )
{
    ...
    {
        arm_dof_index =
                human_active_->getJoint( "rArmTrans" )->getIndexOfFirstDof();
...
void HumanTrajSimulator::setDemonstrations(
        const std::vector<motion_t>& demos )
{
    ...
    for( size_t d=0; d<demos.size(); d++ )
    {
        ...
        cout << "human_2_motions_[" << d << "].size() : "
             << human_2_motions_[d].size() << endl;
...
//            selected.push_back("[7395-7595]_human1_.csv");
    ...
    for( size_t j=0; j<motion_recorders_[0]->getStoredMotions().size(); j++ )
        ...
        {
            cout << "Add motion : " <<
                    motion_recorders_[0]->getStoredMotionName(j) << endl;

            human_1_motions_.push_back(
                        motion_recorders_[0]->getStoredMotions()[j] );
            motions_1_names_.push_back(
                        motion_recorders_[0]->getStoredMotionName(j) );
            human_1_demos_.push_back(
                        human_1_motions_.back() );

            if( is_active_agent_human_ )
            {
                human_2_motions_.push_back(
                            motion_recorders_[1]->getStoredMotions()[j] );
                motions_2_names_.push_back(
                            motion_recorders_[1]->getStoredMotionName(j) );
                human_2_demos_.push_back(
                            human_2_motions_.back() );
            }
...
{
    cout << "size 1 before adding cut motions : "
         << human_1_motions_.size() << endl;
    cout << "size 2 before adding cut motions : "
    ...
    for( size_t i=0; i<human_2_motions_.size(); i++ )
    {
        ...
        cout << "max_nb_of_removed_frames : "
             << max_nb_of_removed_frames << endl;
    ...
    cout << "size 1 after adding cut motions : "
         << human_1_motions_.size() << endl;
...
void HumanTrajSimulator::setPelvisBounds()
{
    ...
    else
    {
        ...
        if( pelvis_min_.size() == 6 && pelvis_max_.size() == 6 )
        {
            ...
            cout << "pelvis_min_.transpose() : "
                 << pelvis_min_.transpose() << endl;
            cout << "pelvis_max_.transpose() : "
                 << pelvis_max_.transpose() << endl;
        }
    }

    for(int i=0; i<6; i++)
        p3d_jnt_set_dof_rand_bounds( pelvis_joint->getP3dJointStruct(),
                                     i, dof[i][0], dof[i][1] );
    ...
    p3d_jnt_set_dof_rand_bounds(
                arm_joint->getP3dJointStruct(), 0,
                arm_min_ - bound_translations,
                arm_max_ + bound_translations );

    if( use_bio_models_ )
    {
        Move3D::Joint* forearm_joint =
                human_active_->getJoint( "rForeArmTrans" );
        p3d_jnt_set_dof_rand_bounds( forearm_joint->getP3dJointStruct(), 0,
        ...
        p3d_jnt_set_dof_rand_bounds(
                    human_active_->getJoint(
                        "rShoulderTransX" )->getP3dJointStruct(),
                    0, dof[0][0], dof[0][1] );
        p3d_jnt_set_dof_rand_bounds(
                    human_active_->getJoint(
                        "rShoulderTransY" )->getP3dJointStruct(),
                    0, dof[1][0], dof[1][1] );
        p3d_jnt_set_dof_rand_bounds(
                    human_active_->getJoint(
                        "rShoulderTransZ" )->getP3dJointStruct(),
                    0, dof[2][0], dof[2][1] );
    }
    ...
    for( size_t i=0; i<human_active_->getNumberOfJoints(); i++)
        p3d_jnt_set_is_user(
                    human_active_->getJoint(i)->getP3dJointStruct(), FALSE );
...
void HumanTrajSimulator::setActiveJoints()
{
    if( is_active_agent_human_)
    {
        if( use_bio_models_ )
        {
            ...
            active_joints_.push_back(
                        human_active_->getJoint( "rShoulderTransX" ) );
            active_joints_.push_back(
                        human_active_->getJoint( "rShoulderTransY" ) );
            active_joints_.push_back(
                        human_active_->getJoint( "rShoulderTransZ" ) );
    ...
    for( size_t i=0; i<active_joints_.size(); i++ ) // get all active dofs
    {
        ...
        active_dofs_.insert( active_dofs_.end(),
                             jnt_active_dofs.begin(),
                             jnt_active_dofs.end() );

        cout << " active joints dof ("
             << active_joints_[i]->getName() << ") [" << i << "] : "
...
double HumanTrajSimulator::getCost( const motion_t& traj ) const
{
    ...
    {
        // cout << " feature at " << i << " : "
        // << cost_space_human_traj_features_->getFeatureFunctionAtIndex(i)->getName() << endl;
    ...
    for (int i=1; i<nb_via_points+1; i++)
    {
            ...
            // set passive human at time along motion
        {
            ...
            {
                Move3D::confPtr_t q =
                        human_1_motions_[id_of_demonstration_][j].second;
                human_passive_->setAndUpdate( *q );
                break;
            }
...
void HumanTrajSimulator::runStandardStomp( int iter )
{
    ...
    if( iter>0 )
    {
        ...
        for(int i=0; i<nb_way_points-1; i++) {
            optimi_traj.push_back(
                        path_.configAtTime(
                            time_along_current_path_ + double(i) * dt ) );
        }
        optimi_traj.push_back(
                    path_.getEnd() );
    ...
    traj_optim_set_iteration_limit(
                PlanEnv->getInt(PlanParam::stompMaxIteration) );
    PlanEnv->setDouble( PlanParam::trajDuration,
                        current_motion_duration_ > 0.0 ?
                            current_motion_duration_ : 0.1  );
    ...
    if( path_.size() > 0 && time_along_current_path_ > 0.0 )
    {
        ...
        for(int i=0; i<nb_config; i++){
            Move3D::confPtr_t q = path_.configAtTime(
                        time_along_current_path_-double(nb_config-i)*dt );
            buffer.push_back( q->getEigenVector( active_dofs_ ) );
        }
        traj_optim_set_buffer( buffer );

        if( !q_init_->equal( *path_.configAtTime(
                                 time_along_current_path_ ) ) ){
    ...
    cout << "path.getUseTimeParameter() : " <<
            path_.getUseTimeParameter() << endl;
}
...
void HumanTrajSimulator::execute(const Move3D::Trajectory& path,
                                 bool to_end)
{
    path.replaceP3dTraj();
    ...
    for( int i=0; i<nb_configs; i++ )
    {
        ...
        executed_trajectory_.push_back(
                    std::make_pair( global_discretization_, q ) );

        // if the time exceeds the trajectory length
        // inferior to a hundredth of a second
        if( ( motion_duration_ - ( t + current_time_ ) )
                < global_discretization_  )
        {
    ...
    {
        while( motion_duration_ - ( t + current_time_ )
               > global_discretization_ )
        {
            t += global_discretization_;
            q = path.configAtTime( t );
...
double HumanTrajSimulator::run()
{
    ...
    cout << "Load human traj id  : " <<
            GestEnv->getInt(GestParam::human_traj_id) << endl;
    ...
    Move3D::Trajectory path( HriEnv->getBool(HricsParam::ioc_no_replanning) ?
                                 path_ :
                                 motion_to_traj(
                                     executed_trajectory_, human_active_ ));
    ...
    cout << "executed_trajectory_.size() : "
         << executed_trajectory_.size() << endl;
    cout << "path.size() : " << path.getNbOfViaPoints() << endl;
    cout << "executed motion duration : "
         << motion_duration( executed_trajectory_ ) << endl;
    cout << "passive motion duration : "
         << motion_duration( human_passive_motion_ ) << endl;
    cout << " active motion duration : "
         << motion_duration( human_active_motion_ ) << endl;
