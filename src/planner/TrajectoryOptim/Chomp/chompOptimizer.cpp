...
// ChompOptimizer::ChompOptimizer(ChompTrajectory *trajectory, const
// ChompRobotModel *robot_model,
//                               const ChompRobotModel::ChompPlanningGroup
//                               *planning_group, const ChompParameters
//                               *parameters,
//                               const ros::Publisher&
...
void ChompOptimizer::initialize() {
  ...
  cout << "Setting free vars start to " << free_vars_start_ << " end "
       << free_vars_end_;
  ...
  for (int i = 0; i < num_joints_; i++) {
    ...
    derivative_costs[1] =
        joint_cost * parameters_->getSmoothnessCostAcceleration();
    derivative_costs[2] = joint_cost * parameters_->getSmoothnessCostJerk();

    joint_costs_.push_back(ChompCost(
        num_vars_all_, i, derivative_costs, parameters_->getRidgeFactor()));
  ...
  //  std::vector<KDL::Vector>(robot_model_->getKDLTree()->getNrOfJoints()));
  //  joint_pos_.resize(num_vars_all_,
  //  std::vector<KDL::Vector>(robot_model_->getKDLTree()->getNrOfJoints()));
  //  segment_frames_.resize(num_vars_all_,
  //  std::vector<KDL::Frame>(robot_model_->getKDLTree()->getNrOfSegments()));
  //  collision_point_pos_.resize(num_vars_all_,
  //  std::vector<KDL::Vector>(num_collision_points_));
  ...
  // std::vector<double>(num_collision_points_));
  // collision_point_vel_mag_.resize(num_vars_all_,
  // std::vector<double>(num_collision_points_));
  ...
  //  kdlVecVecToEigenVecVec(collision_point_pos_, collision_point_pos_eigen_,
  //  3, 1);
  //  kdlVecVecToEigenVecVec(collision_point_vel_, collision_point_vel_eigen_,
  //  3, 1);
  //  kdlVecVecToEigenVecVec(collision_point_acc_, collision_point_acc_eigen_,
  //  3, 1);
  ...
  // robot_model_->getForwardKinematicsSolver()->segmentNameToIndex(parameters_->getAnimateEndeffectorSegment());
}
...
void ChompOptimizer::runDeformation(int nbIteration, int idRun) {
  ...
  double ts(0.0), time = 0.0;
  ChronoOn();

  // iterate
  for (iteration_ = 0; iteration_ < parameters_->getMaxIterations() &&
  ...
  printf("Optimization core finished in %f sec",
...
void ChompOptimizer::calculateCollisionIncrements() {
  ...
  for (int i = free_vars_start_; i <= free_vars_end_; i++) {
    for (int j = 0; j < num_collision_points_; j++) {
      ...
      curvature_vector =
          (orthogonal_projector * collision_point_acc_eigen_[i][j]) /
          vel_mag_sq;
      cartesian_gradient =
      ...
      planning_group_->collision_points_[j].getJacobian(
          joint_pos_eigen_[i],
          joint_axis_eigen_[i],
...
void ChompOptimizer::calculatePseudoInverse() {
  jacobian_jacobian_tranpose_ = jacobian_ * jacobian_.transpose() +
...
void ChompOptimizer::calculateTotalIncrements() {
  for (int i = 0; i < num_joints_; i++) {
    final_increments_.col(i) =
        parameters_->getLearningRate() *
        (joint_costs_[i].getQuadraticCostInverse() *
         (parameters_->getSmoothnessCostWeight() *
...
void ChompOptimizer::getFrames(int segment,
...
void ChompOptimizer::performForwardKinematics() {
  ...
  for (int i = start; i <= end; ++i) {
    ...
    // full_trajectory_->getTrajectoryPointKDL(full_traj_index,
    // kdl_joint_array_);
    full_trajectory_->getTrajectoryPointP3d(full_traj_index, joint_array);

    //    cout << "Trajectory point " << i << " index " << full_traj_index <<
    //    endl;
    ...
    for (int j = 0; j < num_collision_points_; j++) {
      planning_group_->collision_points_[j].getTransformedPosition(
          segment_frames_[i], collision_point_pos_eigen_[i][j]);
      // cout << "coll_point(" << i << " , "  << j << ") = " << endl <<
      // collision_point_pos_eigen_[i][j] << endl;

      // int segment_number =
      // planning_group_->collision_points_[j].getSegmentNumber();
      // collision_point_pos_eigen_[i][j] = segment_frames_[i][segment_number] *
      // planning_group_->collision_points_[j].getPosition();

      double distance;

      bool colliding = collision_space_->getCollisionPointPotentialGradient(
      ...
      //      cout << "collision_point_potential_(" << i << " , "  << j << ") =
      //      " << endl << collision_point_potential_(i,j) << endl;
      //      cout << "collision_point_potential_gradient_(" << i << " , "  << j
      //      << ") = " << endl << collision_point_potential_gradient_[i][j] <<
      //      endl;
  ...
  if (is_collision_free_)  // 2nd turn of verification : test of paths between
                           // configurations
  {
  ...
  // now, get the vel and acc for each collision point (using finite
  // differencing)
  for (int i = free_vars_start_; i <= free_vars_end_; i++) {
    for (int j = 0; j < num_collision_points_; j++) {
      ...
      //      cout << "collision_point_pos_eigen_(" << i << " , "  << j << ") =
      //      " << endl << collision_point_pos_eigen_[i][j] << endl;
      //      cout << "collision_point_potential_(" << i << " , "  << j << ") =
      //      " << endl << collision_point_potential_[i][j] << endl;
      //      cout << "collision_point_potential_gradient_(" << i << " , "  << j
      //      << ") = " << endl << collision_point_potential_gradient_[i][j] <<
      //      endl;
      //      cout << "collision_point_vel_mag_(" << i << " , "  << j << ") = "
      //      << endl << collision_point_vel_mag_[i][j] << endl;
      //      cout << "collision_point_vel_eigen_(" << i << " , "  << j << ") =
      //      " << endl << collision_point_vel_eigen_[i][j] << endl;
      //      cout << "collision_point_acc_eigen_(" << i << " , "  << j << ") =
      //      " << endl << collision_point_acc_eigen_[i][j] << endl;
    }
  }
...
//  // int joint_index =
//  planning_group_->chomp_dofs_[num_joints_-1].kdl_joint_index_;
...
//    if (collision_point_potential_[index][i] <= potential_threshold)
//      scale = 0.0;
//    msg.markers[i].points[1].x -=
//    scale*collision_point_potential_gradient_[index][i].x();
//    msg.markers[i].points[1].y -=
//    scale*collision_point_potential_gradient_[index][i].y();
//    msg.markers[i].points[1].z -=
//    scale*collision_point_potential_gradient_[index][i].z();
