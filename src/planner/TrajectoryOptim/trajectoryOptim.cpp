...
//
// généraliser chomp => init codé en dur et ne tenant pas entièrement compte de
// la GUI
// régler les prob de freeze
// regarder le collision checker : trajectoire valide mais en collision à
// l'éxécution
    ...
    if (m_planning_type == MOBILE_MANIP) {
      m_sce = traj_optim::HumanAwareMobileManip;
             ...
             (global_costSpace->getSelectedCostName() ==
              ...
              global_costSpace->getSelectedCostName() == "NoCost" ||
              global_costSpace->getSelectedCostName() == "costNatural")) {
    if (m_planning_type == MANIPULATION) {
...
bool traj_optim_initScenario() {
  ...
  if (global_costSpace)
    cout << "COST FUNCTION is : " << global_costSpace->getSelectedCostName()
         << endl;
  else
...
bool traj_optim_runChomp() {
  ...
  m_chompplangroup =
      new ChompPlanningGroup(m_robot, traj_optim_get_planner_joints());
  m_chompplangroup->collision_points_ = traj_optim_get_collision_points();

  m_chomptraj =
      new ChompTrajectory(T,
                          DIFF_RULE_LENGTH,
                          *m_chompplangroup,
                          PlanEnv->getDouble(PlanParam::trajDuration));
  m_chomptraj->print();
  cout << "chomp Trajectory has npoints : " << m_chomptraj->getNumPoints()
       << endl;
  cout << "Initialize optimizer" << endl;

  ChompOptimizer optimizer(m_chomptraj,
                           m_chompparams,
                           m_chompplangroup,
                           traj_optim_get_collision_space());
...
bool traj_optim_initStomp() {
  ...
  cout << "Chomp Trajectory has npoints : " << m_chomptraj->getNumPoints()
       << endl;
  cout << "PlanEnv->getDouble(PlanParam::trajDuration) : "
       << PlanEnv->getDouble(PlanParam::trajDuration) << endl;
  cout << "Chomp Trajectory uses time : " << m_chomptraj->getUseTime() << endl;

  cout << "Initialize optimizer" << endl;
  global_optimizer.reset(new stomp_motion_planner::StompOptimizer(
      m_chomptraj,
      m_stompparams,
      m_chompplangroup,
      traj_optim_get_collision_space()));
  ...
  if (PlanEnv->getBool(PlanParam::trajStompWithIterLimit) ||
      m_use_iteration_limit) {
    global_optimizer->setUseIterationLimit(true);
    m_stompparams->max_iterations_ =
  ...
  if (PlanEnv->getBool(PlanParam::trajStompWithTimeLimit)) {
    ...
    m_stompparams->max_time_ =
        PlanEnv->getDouble(PlanParam::trajStompTimeLimit);
  }

  if ((m_sce == traj_optim::HumanAwareManip &&
       m_robot->getName() == "PR2_ROBOT") ||
      (m_sce == traj_optim::HumanSimulation) ||
      (m_sce == traj_optim::CostMap)) {
  ...
  if (m_use_buffer) {
    ...
    if (fct != NULL && fct->getFeatureFunction("SmoothnessAll") != NULL)
      static_cast<SmoothnessFeature*>(fct->getFeatureFunction("SmoothnessAll"))
          ->setBuffer(m_buffer);
  } else {
  ...
  // global_costSpace->addCost("CollisionSpace",
  // boost::bind(computeCollisionSpaceCost,
  // _1));
...
//! Run Stomp
// --------------------------------------------------------
void traj_optim_plan_for_human(bool use) { m_plan_for_human = use; }
...
void traj_optim_set_discretize(bool discretize) { m_discretize = discretize; }

void traj_optim_set_discretization(double discretization) {
  m_discretization = discretization;
}

void traj_optim_clear_buffer() { m_use_buffer = false; }
