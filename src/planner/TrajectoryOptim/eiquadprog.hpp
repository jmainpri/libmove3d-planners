...
structures.
       uQuadProg++ is itself a port made by Angelo Furfaro of QuadProg++
...
template <typename tA,
          ...
          typename tF,
          typename tG>
inline double solve_quadprog(MatrixBase<tA>& G,
                             MatrixBase<tB>& g0,
   ...
   * value for H */
  // J = L^-T
  Eigen::MatrixXd J(G.rows(), G.rows());
  J.setIdentity();
  ...
  return solve_quadprog2(chol, c1, J, c2, g0, CE, ce0, CI, ci0, x);
}
...
inline double solve_quadprog2(LLT<MatrixXd, Lower>& chol,
  ...
  int p = CE.cols();
  int m = CI.cols();
  MatrixXd R(g0.size(), g0.size());

  VectorXd s(m + p), z(n), r(m + p), d(n), np(n), u_tmp(m + p);
  VectorXd x_old(n), u_old(m + p);
  u = u_tmp;
  double f_value, psi, sum, ss, R_norm;
  ...
  * and the full step length t2 */
  ...
  // int q = 0;  /* size of the active set A (containing the indices of the
  // active constraints) */
