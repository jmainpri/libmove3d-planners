...
TrajectoryOptimizer::TrajectoryOptimizer() {
  stomp_parameters_ =
      MOVE3D_BOOST_PTR_NAMESPACE<stomp_motion_planner::StompParameters>(
          new stomp_motion_planner::StompParameters());
...
void TrajectoryOptimizer::runDeformation(int nbIteration, int idRun) {
  ...
  initialize(
      false,
      group_trajectory_.use_time_ ? group_trajectory_.discretization_ : 0.0);
...
SamplingBasedInnerLoop::SamplingBasedInnerLoop(Move3D::Robot* robot)
    : sampler_(robot) {
  robot_model_ = robot;
  ...
  group_trajectory_ = Move3D::VectorTrajectory(
      planning_group_->chomp_dofs_.size(), nb_points, duration);
  ...
  cost_computer_ =
      Move3D::LampCostComputation(robot_model_,
  ...
  if (!use_external_collision_space) {
    cost_computer_.set_fct_get_nb_collision_points(
        boost::bind(&Move3D::LampCostComputation::getNumberOfCollisionPoints,
                    &cost_computer_,
                    _1),
        collision_space_id);
    cost_computer_.set_fct_get_config_collision_cost(
        boost::bind(&Move3D::LampCostComputation::getConfigObstacleCost,
                    &cost_computer_,
                    _1,
                    _2,
                    _3,
...
double SamplingBasedInnerLoop::getTrajectoryCost(Move3D::VectorTrajectory& traj,
...
double SamplingBasedInnerLoop::getStateCost(Move3D::VectorTrajectory& traj,
                                            bool check_joint_limits) {
  ...
  cost_computer_.getCost(
      traj, costs, iteration_, check_joint_limits, false, false);
...
double SamplingBasedInnerLoop::getSmoothnessCost(
  ...
  // return PlanEnv->getDouble(PlanParam::lamp_control_cost) *
  // traj.trajectory_.transpose() * sampler_.control_ * traj.trajectory_;

  const std::vector<Eigen::VectorXd>& control_costs =
      cost_computer_.getControlCosts();
...
Eigen::VectorXd SamplingBasedInnerLoop::generateSamples() {
  std::vector<Move3D::VectorTrajectory> samples =
      sampler_.sampleTrajectories(num_rollouts_gen_, group_trajectory_);
  ...
  if (!rollouts_reused_next_) {
    ...
    if (num_rollouts_reused_ > 0) rollouts_reused_next_ = true;
  } else {
    ...
    for (size_t r = 0; r < samples.size(); ++r) {
      ...
      if (samples[r].out_of_bounds_) cost = std::numeric_limits<double>::max();
    ...
    for (int r = 0; r < num_rollouts_reused_; ++r) {
      ...
      if (reuse_index >= 0) reused_rollouts_[r] = samples[reuse_index];
    }

    // copy them back from reused_rollouts_ into rollouts_
    if (samples.size() >=
        (num_rollouts_gen_ + reused_rollouts_.size()))  //! TODO fix this
    {
      for (int r = 0; r < num_rollouts_reused_; ++r)
