...
void PerturbationRoadmap::getTranslationBounds() {
  p3d_jnt* joint =
      static_cast<p3d_jnt*>(_Robot->getJoint(1)->getP3dJointStruct());
...
void PerturbationRoadmap::addTrajectory(const Move3D::Trajectory& T) {
  ...
  for (double s = m_delta; s < (range_max - m_delta); s += m_delta) {
    ...
    if (PlanEnv->getBool(PlanParam::orientedGraph))
      _Graph->addEdges(
          ...
          node_2,
          false,
          node_1->getConfiguration()->dist(*node_2->getConfiguration()),
          true,
          0);
    else
      _Graph->addEdge(
  ...
  if (PlanEnv->getBool(PlanParam::orientedGraph))
    _Graph->addEdges(node_1,
                     node_2,
                     false,
                     node_1->getConfiguration()->dist(*T.getEnd()),
                     true,
                     0);
  else
    _Graph->addEdge(node_1,
                    node_2,
                    false,
                    node_1->getConfiguration()->dist(*T.getEnd()),
                    true,
...
bool PerturbationRoadmap::findBestCycle(confPtr_t q_new,
  ...
  bool firstNode = true;
  nearNodesQueue.push_back(
      make_pair(numeric_limits<double>::max(), (Node*)NULL));

  for (int i = 0; i < int(_Graph->getNumberOfNodes()); i++) {
    double current_dist = _Graph->getNode(i)->dist(q_new) - dist;

    // If better than last node of the Queue add to the Queue (rewrite better
    // code)
    if (current_dist < nearNodesQueue.back().first) {
      if (firstNode) {
        nearNodesQueue.clear();
        firstNode = false;
      }
  ...
  for (int i = 0; i < int(nearNodesQueue.size()); i++) {
    for (int j = 0; j < int(nearNodesQueue.size()); j++) {
      ...
      vector<Node*> path =
          _Graph->extractAStarShortestNodePaths(q_source, q_target);
...
bool PerturbationRoadmap::testPerturb(confPtr_t q_new,
  ...
  for (int i = 0; i < int(edges.size()); i++) {
    ...
    if (PlanEnv->getBool(PlanParam::orientedGraph))
      _Graph->addEdges(
          vect_nodes[edges[i].first], node_new, false, max_param, true, 0);
    else
      _Graph->addEdge(
          vect_nodes[edges[i].first], node_new, false, max_param, true, 0);
...
bool PerturbationRoadmap::addPerturbation(confPtr_t q_rand) {
  Node* node_near = _Graph->nearestWeightNeighbour(
      m_main_compco, q_rand, ENV.getInt(Env::DistConfigChoice));

  confPtr_t q_new = Move3D::CostOptimization::perturbCurrent(
      node_near->getConfiguration(), q_rand, m_delta, m_descent);

  vector<Node*> nodes = _Graph->KNearestWeightNeighbour(
      q_new, m_K_Nearest, P3D_HUGE, ENV.getInt(Env::DistConfigChoice));

  if (int(nodes.size()) < m_K_Nearest) return false;
...
bool PerturbationRoadmap::expandPerturbation(confPtr_t q_rand) {
  ...
  for (; ith_div < max_div && (is_q_out_of_bounds && is_q_in_collision);
       ith_div++) {
  ...
  if (m_sampled_on_traj) {
    nodes = _Graph->KNearestWeightNeighbour(
        q_new, 5, P3D_HUGE, ENV.getInt(Env::DistConfigChoice));
    // Graph::sortNodesByDist( nodes, q_new );
  } else {
...
confPtr_t PerturbationRoadmap::trajShootConfiguration() {
  ...
  p3d_gaussian_config2_specific(q1->getRobot()->getP3dRobotStruct(),
                                q1->getConfigStruct(),
                                q2->getConfigStruct(),
                                m_std_dev_trans,
                                m_std_dev_rot,
