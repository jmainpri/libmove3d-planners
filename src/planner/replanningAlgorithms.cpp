...
void p3d_deactivate_all_cntrts(Robot* r) {
  ...
  for (int i = 0; i < nb_cntrts; i++) p3d_desactivateCntrt(rob, cntrts[i]);
}
...
Replanner::Replanner(Robot* r) : m_robot(r), m_human(NULL), m_useMLP(false) {
  ...
  if (!init_mlp()) {
    cout << "Error initializing multi-localpahts in " << __FILE__ << " at "
...
void Replanner::setHuman(Robot* hum) { m_human = hum; }

void Replanner::setSwitchData(confPtr_t qSwitch,
                              int switch_id,
                              double s_switch,
                              double t_rep,
...
bool Replanner::init_mlp() {
  ...
  for (int i = 0; i < m_robot->getP3dRobotStruct()->mlp->nblpGp; i++) {
    if (!strcmp(m_robot->getP3dRobotStruct()->mlp->mlpJoints[i]->gpName,
                "base")) {
      m_BaseMLP = i;
      m_useMLP = true;
    } else if (!strcmp(m_robot->getP3dRobotStruct()->mlp->mlpJoints[i]->gpName,
                       "baseSm")) {
      m_BaseSmMLP = i;
      m_useMLP = true;
    } else if (!strcmp(m_robot->getP3dRobotStruct()->mlp->mlpJoints[i]->gpName,
...
std::pair<bool, Move3D::Trajectory> Replanner::concat_to_current_traj(
    const Move3D::Trajectory& newPortion) {
  ...
  Move3D::Trajectory concatTraj =
      m_CurrentTraj.extractSubTrajectory(0, m_s_switch, false);
...
p3d_traj* Replanner::concat_to_current_traj(const vector<p3d_traj*>& trajs) {
  ...
  if (!m_CurrentTraj.isEmpty()) {
    m_CurrentTraj.extractSubTrajectoryOfLocalPaths(0, m_switch_id)
        .replaceP3dTraj();
...
//! Before simple (no softmotion) replanning execution, this function initilizes
//the replanner data
//! First
  ...
  switch (PlanEnv->getInt(PlanParam::replanningInitMethod)) {
    ...
    case 1: {
      if (!p3d_run_rrt(m_robot->getP3dRobotStruct())) {
        ...
        cout << "m_CurrentTraj.getParamMax() : " << m_CurrentTraj.getParamMax()
             << endl;
      }
    } break;
...
void SimpleReplanner::init_for_manipuation() {
  if (m_useMLP) {
    ...
    p3d_multiLocalPath_disable_all_groupToPlan(m_robot->getP3dRobotStruct(),
                                               false);
    p3d_multiLocalPath_set_groupToPlan(
        m_robot->getP3dRobotStruct(), m_UpBodyMLP, 1, false);
#endif
...
void SimpleReplanner::init_for_mobile_manip() {
  ...
  unFixJoint(m_robot->getP3dRobotStruct(),
             m_robot->getP3dRobotStruct()->baseJnt);
...
void SimpleReplanner::init_for_navigation() {
  if (m_useMLP) {
    ...
    p3d_multiLocalPath_disable_all_groupToPlan(m_robot->getP3dRobotStruct(),
                                               false);
    p3d_multiLocalPath_set_groupToPlan(
        m_robot->getP3dRobotStruct(), m_BaseMLP, 1, false);
#endif
...
void SimpleReplanner::run() {
  ...
  Move3D::Trajectory newPortion = m_CurrentTraj.extractSubTrajectory(
      m_s_switch, m_CurrentTraj.getParamMax(), false);
  cout << "newPortion.getNbOfPaths() : " << newPortion.getNbOfPaths() << endl;
  cout << "newPortion.getParamMax() : " << newPortion.getParamMax() << endl;
  ...
  std::pair<bool, Move3D::Trajectory> concated_traj =
      concat_to_current_traj(optimTrj);
...
void RRTReplanner::run() {
  ...
  Move3D::Trajectory traj(m_CurrentTraj.extractSubTrajectory(
      m_s_switch, m_CurrentTraj.getParamMax(), false));
  ...
  if (path != NULL && !PlanEnv->getBool(PlanParam::stopPlanner) &&
...
std::pair<bool, confPtr_t> HandoverReplanner::newGoalFromList() {
  ...
  result.first = handoverGenerator_->computeHandoverConfigFromList(
  ...
  if (result.first) {
    ...
    p3d_update_virtual_object_config_for_arm_ik_constraint(
        m_robot->getP3dRobotStruct(),
        0,
        best_handover_conf.second->getConfigStruct());
...
std::pair<bool, confPtr_t> HandoverReplanner::newGoalFromIK() {
  ...
  // if the generator finds a configuration, return it otherwise return null
  // configuration
  if (handoverGenerator_->computeRobotIkForGrabing(q, point)) {
    ...
    gettimeofday(&tim, NULL);
    dt = tim.tv_sec + (tim.tv_usec / 1000000.0) - t_init;
    cout << "Could not compute a valid robot IK configuration in : " << dt
         << "sec" << endl;
...
void HandoverReplanner::run() {
  ...
  if (path != NULL && !PlanEnv->getBool(PlanParam::stopPlanner) &&
      PlanEnv->getBool(PlanParam::withSmoothing)) {
    p3d_smoothing_function(
        m_robot->getP3dRobotStruct(), path, 100, m_t_rep - 0.3 - time);
  }
  ...
  if (path) {
    ...
    std::pair<bool, Move3D::Trajectory> concated_traj =
        concat_to_current_traj(final_traj);
...
bool StompReplanner::init() { return SimpleReplanner::init(); }
...
void AStarReplanner::run() {
  ...
  Move3D::Trajectory* path_ =
      m_navigation->computeRobotTrajectory(traj.getBegin(), traj.getEnd());
  ...
  if (path_ != NULL && !PlanEnv->getBool(PlanParam::stopPlanner) &&
      PlanEnv->getBool(PlanParam::withSmoothing)) {
    p3d_smoothing_function(
        m_robot->getP3dRobotStruct(), path_->replaceP3dTraj(NULL), 100, -1);
  }
