...
static boost::function<void*(LocalPath&, void*, confPtr_t&, confPtr_t&)>
    Move3DLocalPathCopyFromStruct;
static boost::function<void*(LocalPath&, bool, int&)> Move3DLocalPathGetStruct;
static boost::function<bool(LocalPath&, confPtr_t, int&, double&)>
    Move3DLocalPathClassicTest;
...
static boost::function<confPtr_t(LocalPath&, double)>
    Move3DLocalPathConfigAtDist;
static boost::function<void(LocalPath&, double, confPtr_t&)>
    Move3DLocalPathConfigAtParam;
static boost::function<double(LocalPath&, double, bool, double&)>
    Move3DLocalPathStayWithinDist;

// ****************************************************************************************************
// SETTERS
// ****************************************************************************************************

void move3d_set_fct_localpath_copy(
    boost::function<void*(const LocalPath&, Robot*)> fct) {
  Move3DLocalPathCopy = fct;
}
void move3d_set_fct_localpath_copy_p3d(
    boost::function<void*(const LocalPath&, Robot*)> fct) {
  Move3DLocalPathCopyP3d = fct;
}
void move3d_set_fct_localpath_path_destructor(
    boost::function<void(LocalPath&)> fct) {
  Move3DLocalPathDestructor = fct;
}
void move3d_set_fct_localpath_copy_from_struct(
    boost::function<void*(LocalPath&, void*, confPtr_t&, confPtr_t&)> fct) {
  Move3DLocalPathCopyFromStruct = fct;
}
void move3d_set_fct_localpath_get_struct(
    boost::function<void*(LocalPath&, bool, int&)> fct) {
  Move3DLocalPathGetStruct = fct;
}
void move3d_set_fct_localpath_classic_test(
    boost::function<bool(LocalPath&, confPtr_t, int&, double&)> fct) {
  Move3DLocalPathClassicTest = fct;
}
void move3d_set_fct_localpath_is_valid(
    boost::function<bool(LocalPath&, int&)> fct) {
  Move3DLocalPathIsValid = fct;
}
void move3d_set_fct_localpath_get_length(
    boost::function<double(LocalPath&)> fct) {
  Move3DLocalPathGetLength = fct;
}
void move3d_set_fct_localpath_get_param_max(
    boost::function<double(LocalPath&)> fct) {
  Move3DLocalPathGetParamMax = fct;
}
void move3d_set_fct_localpath_config_at_dist(
    boost::function<confPtr_t(LocalPath&, double)> fct) {
  Move3DLocalPathConfigAtDist = fct;
}
void move3d_set_fct_localpath_config_at_param(
    boost::function<void(LocalPath&, double, confPtr_t&)> fct) {
  Move3DLocalPathConfigAtParam = fct;
}
void move3d_set_fct_localpath_stay_within_dist(
    boost::function<double(LocalPath&, double, bool, double&)> fct) {
  Move3DLocalPathStayWithinDist = fct;
}
...
LocalPath::~LocalPath() { Move3DLocalPathDestructor(*this); }

// Accessors
p3d_localpath* LocalPath::getP3dLocalpathStruct(bool multi_sol) {
  return static_cast<p3d_localpath*>(
      Move3DLocalPathGetStruct(*this, multi_sol, _Type));
}

confPtr_t LocalPath::getBegin() { return _Begin; }

confPtr_t LocalPath::getEnd() { return _End; }

Robot* LocalPath::getRobot() { return _Robot; }

bool LocalPath::getEvaluated() { return _Evaluated; }
...
bool LocalPath::classicTest() {
  ...
  if (!_lastValidEvaluated) {
    ...
    _Valid = Move3DLocalPathClassicTest(
        *this, _lastValidConfig, _NbColTest, _lastValidParam);
...
double LocalPath::length() { return Move3DLocalPathGetLength(*this); }

double LocalPath::getParamMax() { return Move3DLocalPathGetParamMax(*this); }
...
double LocalPath::stayWithInDistance(double u,
                                     bool goForward,
                                     double* distance) {
...
//{
//    return p3d_unvalid_localpath_test( (p3d_rob*)R->getP3dRobotStruct(),
//    getP3dLocalpathStruct(), ntest );
//}
...
double LocalPath::getResolution(double step) {
  if (!_ResolEvaluated) {
    ...
    if (step == 0.0) {
      step = PlanEnv->getDouble(PlanParam::costResolution) *
             ENV.getDouble(Env::dmax);
    }
...
vector<pair<double, double> > LocalPath::getCostProfile() {
  ...
  for (unsigned int i = 0; i < nStep; i++) {
    ...
    //		cout << "vectOfCost[" << i << "].first = " << vectOfCost[i].first <<
    //endl;
    //		cout << "vectOfCost[" << i << "].second = " << vectOfCost[i].second <<
    //endl;
  }
...
double LocalPath::whenCostIntegralPasses(double thresh) {
  ...
  for (unsigned int i = 0; i < nStep; i++) {
    ...
    costSoFar +=
        global_costSpace->deltaStepCost(prevCost, currentCost, DeltaStep);
