...
unsigned int Graph::getNumberOfNodes()
{
    if( m_initBGL ) {
        ...
        {
            throw string("Vertices in boost graph don't reflect the C++ graph");
        }
        return nbNodes;
    }
    else {
     ...
     double currentCost, double step)*/
{
    ...
    if( !isInGraph(expansionNode) )
    {
        ...
        {
            throw string("expansionNode is not in the graph"); //configuration space
        }
    }
...
/**
 * Compare Nodes																//Impose calcul à une config commune au préalable
 */
...
vector<Edge*> Graph::addEdges(Node* N1, Node* N2, bool compute_length, double length , bool compute_cost,  double cost )
{
    ...
    edges[1] = addEdge(N2, N1, false, edges[0]->longueur(), false, edges[0]->cost() );
...
/**
 * Add Edge to all node inferior to max Dist from N
 */
...
void Graph::removeNode(Node* to_del_node, bool rebuild_compco)
{
    ...
    {
        throw string("to_del_node is not in the graph");
    }
...
bool Graph::linkNodeWithoutDist(Node* N)
{
    ...
    for (int i=0; i<int(m_Comp.size()); i++)
    {
        if( N->getConnectedComponent() != m_Comp[i] )
        {
            ...
            {
                if (linkNodeAndMerge(nodes[j],N,true))
                {
                    nof_link++;
                    break;
                }
...
bool Graph::linkNodeCompMultisol( Node* N, ConnectedComponent* TargetComp )
{
    ...
    for (int i=0; i<int(nodes.size()); i++)
    {
        ...
        if (p3d_get_SORTING() == P3D_DIST_NODE) {
            if ((Nc->getNodeStruct()->dist_Nnew > p3d_get_DMAX()) && (p3d_get_DMAX() > 0.)) { //error here
                return false;
            }
        }
    ...
    /* No connexion has been found (in oriented case, arcs may have been created) */
    return false;
}
...
bool Graph::connectNodeToCompco(Node* node1, Node* compco)
{
    ...
    {
        cout << "Warning: Try to connect a Node to its own component" << endl;
        return true;
    }
...
void Graph::addCycles(Node* node, double step)
{
    ...
    p3d_list_node* listDistNodePt = p3d_listNodeInDist( m_Robot->getRobotStruct(),
                                                        node->getNodeStruct()->comp,
                                                        node->getNodeStruct(),
                                                        longStep);
