...
BodySurfaceSampler::BodySurfaceSampler(double step, double clearance)
    : m_step(step), m_collision_clearance_default(clearance) {}
...
PointCloud& BodySurfaceSampler::sampleObjectSurface(p3d_obj* obj,
                                                    bool isRobot) {
    ...
    if (obj->pol[i]->TYPE != P3D_GRAPHIC) {
      ...
      for (unsigned int j = 0; j < poly->nb_faces; j++) {
        ...
        p3d_vector3* tmp = sample_triangle_surface(
            ...
            the_points[poly->the_faces[j].the_indexs_points[2] - 1],
            m_step,
            &nbPoints);
        ...
        for (unsigned int k = 0; k < nbPoints; k++) {
          ...
          if (isRobot ||
              (isPointInEnvironment(point) && isPointOverGround(point))) {
...
void BodySurfaceSampler::sampleObjectInside(p3d_obj* obj,
  ...
  for (int i = 0; i < obj->np; i++) {
    if (obj->pol[i]->entity_type) {
      cout << "body name " << obj->name
           << " , entity type : " << obj->pol[i]->entity_type << endl;

      if ((obj->pol[i]->entity_type != 5) && (obj->pol[i]->entity_type != 6))
        continue;

      // Seems to be the same as pos_rel_jnt
      // if (obj->type == P3D_OBSTACLE) {
      //  // real point position
      //  p3d_mat4Copy(obj->pol[i]->pos0, mat);
      // } else {
      //  p3d_matrix4 inv_pos;
      //  p3d_matInvertXform(obj->jnt->pos0, inv_pos);
      //  p3d_matMultXform(inv_pos, obj->pol[i]->pos0, mat);
      ...
      for (int j = 0; j < 20000; j++) {
        if (obj->pol[i]->entity_type == 5)  // box
        {
          ...
          p_o[0] = p3d_random(-x / 2, x / 2);
          p_o[1] = p3d_random(-x / 2, y / 2);
          p_o[2] = p3d_random(-z / 2, z / 2);
        }
        if (obj->pol[i]->entity_type == 6)  // cylinder
        {
          double r = obj->pol[i]->primitive_data->radius;
          double h = obj->pol[i]->primitive_data->height;

          double rand_r = p3d_random(0, r);
          double rand_phi = p3d_random(0, 2. * M_PI);
          double rand_h = p3d_random(-h / 2, h / 2);

          p_o[0] = rand_r * cos(rand_phi);
          p_o[1] = rand_r * sin(rand_phi);
          p_o[2] = rand_h;
        }

        p3d_vector3 p;
        p3d_xformPoint(obj->pol[i]->pos_rel_jnt, p_o, p);
...
void BodySurfaceSampler::sampleRobotBodiesSurface(Robot* robot) {
  ...
  for (int i = 0; i < int(robot->getNumberOfJoints()); i++) {
    p3d_obj* obj =
        static_cast<p3d_jnt*>(robot->getJoint(i)->getP3dJointStruct())->o;
...
BoundingCylinder* BodySurfaceSampler::generateBoudingCylinder(p3d_obj* obj) {
  ...
  if (pqp_get_OBB_first_level(obj, box)) {
    for (unsigned int i = 0; i < 8; i++)
      for (unsigned int j = 0; j < 3; j++) cuboide[i][j] = box[i][j];
...
double BodySurfaceSampler::generateRobotBoudingCylinder(
    Robot* robot, const vector<Joint*>& activeJoints) {
  double maxRadius = numeric_limits<double>::min();

  for (unsigned int i = 0; i < activeJoints.size(); i++) {
    p3d_obj* obj =
        static_cast<p3d_jnt*>(activeJoints[i]->getP3dJointStruct())->o;
...
std::vector<CollisionPoint>& BodySurfaceSampler::getCollisionPoints(
    Joint* jnt) {
...
std::vector<CollisionPoint> BodySurfaceSampler::getLinksCollisionPoints(
    Joint* jnt, int segment_number, const std::vector<int>& parent_joints) {
  std::vector<CollisionPoint> collision_points;

  if (m_objectToBoCylinderMap.empty()) return collision_points;

  BoundingCylinder* bc =
      m_objectToBoCylinderMap[static_cast<p3d_jnt*>(jnt->getP3dJointStruct())
                                  ->o];

  if (bc == NULL) return collision_points;
  ...
  // cout << "segment id : " << segment_number << " , nb of coll points : " <<
  // num_points << endl;
  ...
  for (int i = 0; i < num_points; ++i) {
    ...
    collision_points.push_back(CollisionPoint(parent_joints,
                                              radius,
...
std::vector<CollisionPoint> BodySurfaceSampler::generateJointCollisionPoints(
    Robot* robot,
    int id,
  ...
  // here it counts the number of DoFs for each planned joint to retreive the
  // index
  for (unsigned int i = 0; i < planner_joints.size(); i++) {
    for (unsigned int j = 0;
         j < robot->getJoint(planner_joints[i])->getNumberOfDof();
         j++) {
      if (!robot->getJoint(planner_joints[i])->isJointDofUser(j)) continue;
    ...
    if (joint == planner_joints[i]) break;
  }
  ...
  //    {
  //        cout << "segment : " << id << " is replaced by : " <<
  //        planner_joints.size()-1 ;
  ...
  std::vector<CollisionPoint> points =
      getLinksCollisionPoints(jnt, segment, parent_joints);
...
std::vector<CollisionPoint> BodySurfaceSampler::generateAllRobotCollisionPoints(
    Robot* robot) {
...
//! Generate the collision points for links associated to the active joints
//! It computes the parent joints of the joint
std::vector<CollisionPoint> BodySurfaceSampler::generateRobotCollisionPoints(
    Robot* robot,
    const std::vector<int>& active_joints,
    const std::vector<int>& planner_joints,
  ...
  for (unsigned int j = 0; j < sc->getNumberOfRobots(); j++) {
    ...
    for (unsigned int i = 0; i < rob->getNumberOfJoints(); i++) {
      ...
      if (obj) {
        ...
        if (rob->getName().find("PR2") == string::npos) continue;
