...
CollisionPoint::CollisionPoint(const std::vector<int>& parent_joints,
                               double radius,
                               double clearance,
                               int segment_number,
...
void CollisionPoint::getJacobian(
  ...
  for (unsigned int joint = 0; joint < group_joint_to_move3d_joint_index.size();
    ...
    if (!isParentJoint(group_joint_to_move3d_joint_index[joint])) {
      ...
      jacobian.col(joint) =
          joint_axis[joint].cross(collision_point_pos - joint_pos[joint]);
...
BoundingCylinder::BoundingCylinder(const Eigen::Vector3d& vect1,
