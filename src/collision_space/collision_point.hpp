...
class BoundingCylinder {
 public:
  ...
  BoundingCylinder(const Eigen::Vector3d& p1,
                   const Eigen::Vector3d& p2,
                   double radius)
      : m_p1(p1), m_p2(p2), m_radius(radius) {}

  BoundingCylinder(const Eigen::Vector3d& vect1,
                   const Eigen::Vector3d& vect2,
                   const Eigen::Vector3d& vect5,
...
class CollisionPoint {
 public:
  ...
  CollisionPoint(const std::vector<int>& parent_joints,
                 double radius,
                 double clearance,
                 int segment_number,
  ...
  const std::vector<int>& getParentJoints() const { return m_parent_joints; }
 ...
 private:
  ...
  Eigen::Vector3d
      m_position; /**< Vector of this point in the frame of the above segment */
};

inline bool CollisionPoint::isParentJoint(int joint) const {
  return (find(m_parent_joints.begin(), m_parent_joints.end(), joint) !=
          m_parent_joints.end());
}

inline double CollisionPoint::getRadius() const { return m_radius; }

inline double CollisionPoint::getVolume() const { return m_volume; }

inline double CollisionPoint::getClearance() const { return m_clearance; }

inline double CollisionPoint::getInvClearance() const {
  return m_inv_clearance;
}

inline int CollisionPoint::getSegmentNumber() const { return m_segment_number; }
